...
def parse_args():
    ...
    parser.add_argument('--trainset_dir', type=str, default='/SRdata/lhsDATA/DATA/data_for_training/SR_5x5_4x_16/')
    ...
    parser.add_argument('--parrallel', type=int, default=0)  #是否使用多个GPU
    ...
    parser.add_argument('--model_name', type=str, default='LFMT1.71')
    parser.add_argument('--load_pretrain', type=int, default=1)
    parser.add_argument('--model_path', type=str, default='') #预训练模型路径？
...
def train(cfg, train_loader, test_Names, test_loaders):
    ...
    if cfg.load_pretrain:
        if os.path.isfile(cfg.model_path):
            model = torch.load(cfg.model_path, map_location={'cuda:0': cfg.device}) #从哪加载权重？
